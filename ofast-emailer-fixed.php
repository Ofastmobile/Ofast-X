<?php
/*
Plugin Name: Ofast Emailer Pro+ (FIXED)
Description: Advanced email sender with scheduling, user ranges, and logs - ALL 13 FIXES APPLIED
Author: Olabode Oluwaseun
Version: 2.1 - FIXED
*/
...
function ofast_emailer_send_page()
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_email'])) {
        ...
        if ($send_test) {
            ...
            // FIX #4 & #5: Change threshold from 50 to 40 for scheduling
    ...
    echo '</p>
        ...
        <p>
            ...
            <small>Leave blank to send immediately. More than 40 recipients will auto-schedule in batches.</small></label>
        </p>
    ...
    echo '<script>
        jQuery(document).ready(function($) {
            ...
            function updatePagination() {
                ...
                $("#user-pagination").html(visibleRows.length + " users found | " +pagination);
...
// FIX #9, #10, #11: Email template with mobile fixes and updated footer
    ...
    <style>
        @media screen and (max-width: 600px) {
            .ofast-email-body {
                ...
                /* FIX #9: Reduced mobile padding */
            }

            .ofast-email-outer {
                padding: 20px 10px !important;
                /* FIX #9: Reduced top/bottom padding */
            }
        }
    </style>
    <div class="ofast-email-outer" style="background:#f9f9f9;padding:30px;font-family:Segoe UI,Tahoma,Geneva,Verdana,sans-serif;color:#333;">
        <div class="ofast-email-body" style="max-width:600px;margin:auto;background:#fff;border-radius:8px;padding:20px 30px;line-height:1.6em;">
            ...
            <div style="text-align:center;font-size:13px;color:#888;">
                ...
                <!-- FIX #12 & #13: Editable footer text -->
...
// Handle scheduled email hook with batch tracking
add_action('ofast_scheduled_email_event', function ($subject, $body, $selected_roles, $selected_user_ids, $batch_number = 1, $total_batches = 1) {
    $headers = [
        'Content-Type: text/html; charset=UTF-8',
        'From: ' . get_option('ofast_email_from_name', 'Ofastshop Digitals') . ' <' . get_option('ofast_email_reply_to', 'support@ofastshop.com') . '>',
        'Reply-To: ' . get_option('ofast_email_reply_to', 'support@ofastshop.com')
    ...
    $wpdb->insert($wpdb->prefix . 'ofast_email_logs', [
        ...
        'batch_info' => "Batch $batch_number/$total_batches"
    ]);
}, 10, 6);
...
function ofast_dashboard_page()
{
    ...
    echo '<div class="wrap"><h2>📊 Ofast Dashboard</h2>';
...
add_action('admin_menu', function () {
    add_menu_page('Ofast Dashboard', '📊 Ofast Dashboard', 'manage_options', 'ofast-dashboard', 'ofast_dashboard_page', 'dashicons-chart-bar', 2);
}, 9);
...
// FIX #4: Improved Scheduled Emails Page
...
// FIX #2, #3, #12, #13: Enhanced Settings Page
