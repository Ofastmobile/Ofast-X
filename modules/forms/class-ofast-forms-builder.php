...
?>
                    ...
                    <div class="ofast-builder-main">
                        ...
                        <div class="ofast-card">
                            ...
                            <table class="form-table">
                                 ...
                                 *
                                 * The mirror text inputs (hex strings for keyboard entry) are
                                 * UI-only helpers. They have NO `name` attribute so they are
                                 * never submitted and never stored. The <input type="color">
                                 * is the sole authoritative input that carries the name and
                                 * gets submitted. JS below keeps the two in sync for the user.
                                 */
                                $color_fields = array(
                                    'btn_bg'      => array('label' => 'Button Background',       'default' => '#6366f1'),
                                    'btn_text'    => array('label' => 'Button Text Color',        'default' => '#ffffff'),
                                    'btn_hover'   => array('label' => 'Button Hover Background',  'default' => '#4f46e5'),
                                    'form_bg'     => array('label' => 'Form Background',           'default' => '#ffffff'),
                                    'input_border'=> array('label' => 'Input Border Color',       'default' => '#dddddd'),
                                    'input_focus' => array('label' => 'Input Focus Color',        'default' => '#6366f1'),
                                );
                                foreach ($color_fields as $key => $meta):
                                    $val = esc_attr($design[$key] ?? $meta['default']);
                                ?>
                                <tr>
                                    <th><?php echo esc_html($meta['label']); ?></th>
                                    <td>
                                        <input type="color" name="settings[design][<?php echo $key; ?>]" value="<?php echo $val; ?>" data-color-key="<?php echo $key; ?>">
                                        <!-- No name: UI-only mirror input, never submitted -->
                                        <input type="text" value="<?php echo $val; ?>" style="width:80px;" class="color-mirror" data-color-key="<?php echo $key; ?>">
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                            </table>
        ...
        <script>
            jQuery(function($) {
                ...
                /*
                 * FIX #7: Guard against double-init if forms-admin.js also calls
                 * sortable (jQuery UI silently ignores the second call, but the
                 * guard keeps intent explicit and avoids any edge-case weirdness).
                 */
                ...
                // --- FIX #9/#14: Color picker ↔ mirror text-input sync ---
                // The mirror inputs have NO name attribute so they are never
                // submitted. The color picker is the sole form value.

                $(document).on('input change', 'input[type="color"][data-color-key]', function() {
                    var key = $(this).data('color-key');
                    $('input.color-mirror[data-color-key="' + key + '"]').val($(this).val());
                });

                $(document).on('input change', 'input.color-mirror[data-color-key]', function() {
                    var val = $(this).val();
                    if (/^#[0-9A-Fa-f]{6}$/.test(val)) {
                        var key = $(this).data('color-key');
                        $('input[type="color"][data-color-key="' + key + '"]').val(val);
                    }
                });
